...
void Summ_Proizvodn::gen_all_proizvodn(const std::string &path,
...
//void  Summ_Proizvodn::gen_tab_summ_proizvodn(std::vector<double> pro_host_other, const Summ_Proizvodn& other)
//{
    ...
    for (int k = 0; k <(pro_host.size() / (other._all_host_number-1)); ++k)  // в потоке нужно знать кол-во хостов в интервале
    {
        double summa = 0; // сюда суммируются производные
        for (int f = 0; f <_host_num_for_one; ++f)   // по количеству даных на 1 хост
        {
            summa += pro_host[k+(f*(pro_host.size() / (other._all_host_number)))];      //(pro_host_other.size() /_host_num_for_one))];
        }
